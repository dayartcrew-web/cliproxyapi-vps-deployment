...
verify_uninstallation() {
    ...
    if docker ps -a | grep -q "cli-proxy-api"; then
    ...
    if [[ $issues -eq 0 ]]; then
        ...
        print_warning "$issues issue(s) detected. Manual cleanup may be required."
    fi
}
...
deep_uninstall_mode() {
    ...
    cd $INSTALL_DIR/CLIProxyAPI 2>/dev/null && docker compose down -v 2>/dev/null || true
